...
Eigen::MatrixXd matrix_to_Eigen(const nifti_matrix *mat) {
...
void Eigen_to_matrix(const Eigen::MatrixXd &eigen_mat, nifti_matrix *mat) {
...
extern "C" {
    ...
    void loadNthSlice_c(const char* nii_filename, nifti_matrix* slice, int i) {
        Eigen::MatrixXd eigen_slice;
        loadNthSlice(std::string(nii_filename), eigen_slice, i);
        Eigen_to_matrix(eigen_slice, slice);
    }
}